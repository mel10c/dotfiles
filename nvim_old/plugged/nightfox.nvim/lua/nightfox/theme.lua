...
function M.apply(colors, config)
  ...
  theme.groups = {
    ...
    Conceal = { fg = c.blue_dm }, -- placeholder characters substituted for concealed text (see 'conceallevel')
    ...
    CursorLineNr = { fg = c.blue_br }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    ...
    PmenuSel = { fg = c.blue_br, bg = util.darken(c.fg_gutter, 0.8) }, -- Popup menu: selected item.
    ...
    IncSearch = config.inverse.search and { style = "inverse" } or { bg = c.blue_br, fg = c.black }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    ...
    SpellBad = { fg = c.orange_dm, style = "undercurl" }, -- Word that is not recognized by the spellchecker. |spell| Combined with the highlighting used otherwise.
    ...
    Identifier = { fg = c.blue_br, style = config.styles.variables }, -- (preferred) any variable name
